...
`@sr-herz/claude-crap` ships its deterministic engines as an npm
...
| Subpath | Exports |
| --- | --- |
| `@sr-herz/claude-crap` | Root re-export of the most common symbols plus every type |
| `@sr-herz/claude-crap/metrics` | `computeCrap`, `computeTdr`, `classifyTdr`, `ratingIsWorseThan`, `ratingToRank`, `computeProjectScore`, `renderProjectScoreMarkdown`, `estimateWorkspaceLoc`, `MAX_FILES_WALKED` |
| `@sr-herz/claude-crap/sarif` | `SarifStore`, `buildSarifDocument`, every `Sarif*` type |
| `@sr-herz/claude-crap/ast` | `TreeSitterEngine`, `computeCyclomaticComplexity`, `detectLanguageFromPath`, `LANGUAGE_TABLE` |
| `@sr-herz/claude-crap/tools` | `findTestFile`, `isTestFile`, `candidatePaths` |
| `@sr-herz/claude-crap/adapters` | `adaptScannerOutput`, `adaptSemgrep`, `adaptEslint`, `adaptBandit`, `adaptStryker`, `KNOWN_SCANNERS`, `DEFAULT_EFFORT_BY_SEVERITY`, `wrapResultsInSarif`, `estimateEffortMinutes` |

Prefer deep imports (e.g. `@sr-herz/claude-crap/metrics`) over the
...
```ts
import { computeCrap } from "@sr-herz/claude-crap/metrics";
...
```ts
import { computeTdr, classifyTdr } from "@sr-herz/claude-crap/metrics";
...
```ts
import { SarifStore } from "@sr-herz/claude-crap/sarif";
import { adaptSemgrep } from "@sr-herz/claude-crap/adapters";
...
```ts
import { TreeSitterEngine } from "@sr-herz/claude-crap/ast";
...
```ts
import { findTestFile } from "@sr-herz/claude-crap/tools";
...
import type { AdapterResult, KnownScanner } from "@sr-herz/claude-crap/adapters";
```

The root `@sr-herz/claude-crap` entry re-exports every type too,
