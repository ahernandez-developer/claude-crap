...
var DEFAULT_EFFORT_BY_SEVERITY = Object.freeze({
  error: 60,
  warning: 30,
  note: 10,
  none: 5
});
function wrapResultsInSarif(sourceTool, version, results) {
  return {
...
function estimateEffortMinutes(level, override) {
...
var SEMGREP = "semgrep";
var SEMGREP_EFFORT_OVERRIDES = /* @__PURE__ */ new Map([
  [/security\./i, 90],
  [/sqli|xss|ssrf|rce|deserial|crypto/i, 120],
  [/style\./i, 5],
  [/formatting\./i, 3]
]);
function adaptSemgrep(input) {
  ...
  for (const run of runs) {
    ...
    for (const result of results) {
      ...
      const existingProps = result["properties"] && typeof result["properties"] === "object" ? result["properties"] : {};
      result["properties"] = {
        ...existingProps,
        sourceTool: SEMGREP,
    ...
    for (const msg of messages) {
      ...
      results.push(
        buildSarifResult({
          ...
          startLine: line,
          startColumn: column,
          endLine: typeof msg.endLine === "number" ? msg.endLine : void 0,
          endColumn: typeof msg.endColumn === "number" ? msg.endColumn : void 0,
          effortMinutes: effort
        })
      );
    }
    ...
    locations: [
      {
        physicalLocation: {
          ...
          region: {
            ...
            ...opts.endLine !== void 0 ? { endLine: opts.endLine } : {},
            ...opts.endColumn !== void 0 ? { endColumn: opts.endColumn } : {}
          }
        }
      }
    ],
...
// src/adapters/bandit.ts
var BANDIT = "bandit";
function adaptBandit(input) {
  const parsed = typeof input === "string" ? JSON.parse(input) : input;
  if (!parsed || typeof parsed !== "object") {
  ...
  for (const finding of report.results) {
    ...
    const level = mapSeverity2(finding.issue_severity);
    ...
    const messageText = finding.issue_text ?? `${finding.test_name ?? "Bandit finding"} (${finding.issue_severity ?? "UNKNOWN"})`;
    const startLine = typeof finding.line_number === "number" && finding.line_number > 0 ? finding.line_number : 1;
    const startColumn = typeof finding.col_offset === "number" && finding.col_offset >= 0 ? finding.col_offset + 1 : 1;
    results.push(
      buildSarifResult2({
        ruleId,
        level,
...
function buildSarifResult2(opts) {
  return {
    ruleId: opts.ruleId,
    level: opts.level,
    message: { text: opts.message },
...
function adaptStryker(input) {
  ...
  for (const [filename, fileReport] of Object.entries(report.files)) {
    ...
    for (const mutant of mutants) {
      ...
      const message = `${statusText}: ${mutator} mutant on '${mutant.replacement ?? "?"}'` + (mutant.statusReason ? ` \u2014 ${mutant.statusReason}` : "");
      results.push(
        buildSarifResult3({
          ruleId,
          level,
          ...
          endColumn: typeof endColumn === "number" ? endColumn : void 0,
          effortMinutes: effort,
          mutantId: mutant.id,
          mutator,
...
function buildSarifResult3(opts) {
  return {
    ruleId: opts.ruleId,
    level: opts.level,
    message: { text: opts.message },
...
// src/ast/cyclomatic.ts
function computeCyclomaticComplexity(root, languageConfig) {
  let complexity = 1;
...
function isBooleanExpression(node, languageConfig) {
  if (node.type !== "binary_expression" && node.type !== "boolean_operator" && node.type !== "logical_expression") {
    return false;
  }
  for (let i = 0; i < node.childCount; i++) {
    const child = node.child(i);
...
// src/ast/language-config.ts
var CSHARP = {
  id: "csharp",
  wasmName: "tree-sitter-c_sharp.wasm",
  extensions: [".cs"],
  functionNodeTypes: /* @__PURE__ */ new Set([
    "method_declaration",
  ...
  branchingNodeTypes: /* @__PURE__ */ new Set([
    "if_statement",
    "else_clause",
    "while_statement",
    "do_statement",
...
var TYPESCRIPT = {
  ...
  branchingNodeTypes: /* @__PURE__ */ new Set([...JAVASCRIPT.branchingNodeTypes]),
...
var PYTHON = {
  ...
  extensions: [".py", ".pyi"],
  functionNodeTypes: /* @__PURE__ */ new Set(["function_definition", "lambda"]),
  branchingNodeTypes: /* @__PURE__ */ new Set([
    "if_statement",
    "elif_clause",
    "else_clause",
...
var TreeSitterEngine = class {
  ...
  async initParserModule() {
    ...
    await Parser.init({
      locateFile: (name) => name === "tree-sitter.wasm" ? join(this.runtimeDir, name) : join(this.grammarsDir, name)
    });
...
function computeProjectScore(input) {
  ...
  for (const finding of findingsList) {
    ...
    const effort = typeof finding.properties?.effortMinutes === "number" ? finding.properties.effortMinutes : 0;
...
function renderProjectScoreMarkdown(score) {
  ...
  return [
    ...
    `| Reliability     |   ${score.reliability.rating}    | ${score.reliability.errorFindings} error \xB7 ${score.reliability.warningFindings} warning \xB7 ${score.reliability.noteFindings} note |`,
    `| Security        |   ${score.security.rating}    | ${score.security.errorFindings} error \xB7 ${score.security.warningFindings} warning \xB7 ${score.security.noteFindings} note |`,
    ``,
    `Workspace: **${score.loc.physical} LOC** across **${score.loc.files} files**`,
    `Findings:  **${score.findings.total} total** (${score.findings.error} error \xB7 ${score.findings.warning} warning \xB7 ${score.findings.note} note)`,
    `Tools:     ${Object.keys(score.findings.byTool).join(", ") || "<none ingested>"}`,
    ``,
    dashboardLine,
    `\u{1F4C4} Report:      ${score.location.sarifReportPath}`
  ].join("\n");
}
...
async function startDashboard(options) {
  ...
  const fastify = Fastify({
    logger: false,
    // we route everything through pino-on-stderr ourselves
...
async function resolvePublicRoot(logger2) {
...
// src/metrics/crap.ts
function computeCrap(input, threshold) {
          ...
          locations: [
            {
              physicalLocation: {
                ...
                region: {
                  ...
                  ...f.location.endLine !== void 0 ? { endLine: f.location.endLine } : {},
                  ...f.location.endColumn !== void 0 ? { endColumn: f.location.endColumn } : {}
                }
              }
            }
          ],
...
`
          ...
          for (const result of run.results) {
            try {
              ...
              process.stderr.write(
                `[sarif-store] ${this.filePath}: dropping malformed result: ${entryErr.message}
`
              );
            }
          }
          ...
          process.stderr.write(
            `[sarif-store] ${this.filePath}: dropping malformed run: ${runErr.message}
`
          );
        }
      }
...
function hydrateFindingFromResult(result, sourceTool) {
  ...
  return {
    ...
    location: {
      ...
      ...region.endLine !== void 0 ? { endLine: region.endLine } : {},
      ...region.endColumn !== void 0 ? { endColumn: region.endColumn } : {}
    },
...
var SarifValidationError = class extends Error {
  errors;
...
function getValidator() {
  ...
  const ajv = new import_ajv.Ajv({ allErrors: false, strict: false });
  const validator = ajv.compile(SARIF_MINIMAL_SCHEMA);
  cachedValidator = validator;
  return validator;
}
function validateSarifDocument(doc) {
